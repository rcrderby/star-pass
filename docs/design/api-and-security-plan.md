...
| `POST /v1/runs/{id}/revisions`, `…/revisions/{n}/revert` | Seal a revision; revert to one (sealing first, never destructive). Reverting to revision 1 also drops hand-added events and returns them to Not collected. |
