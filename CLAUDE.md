...
- `docs/architecture.md` — the deployment and the paths a run takes
  across it, in two pictures. The SVG files beside it are generated;
  edit the source and run the command rather than editing an SVG. What
  is in a picture is `scripts/generate_architecture.py`, which is also
  the command: the boxes, the networks, the arrows and the coordinates
  each sits at. How one is drawn is `scripts/_drawing.py` beside it,
  which knows nothing about this application. Change a label or move a
  box in the first; change what an arrow or a box looks like in the
  second. `tests/test_architecture.py` fails while a committed picture
